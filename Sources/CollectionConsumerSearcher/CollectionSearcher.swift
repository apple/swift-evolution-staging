public protocol CollectionSearcher {
  ...
  /// Generates an instance of state for the given collection.
  func preprocess(_ c: C) -> State

  /// Returns a range of the first match within `c[s...]`, if one exists.
  ///
  /// `state` *must* originally have been created by calling `preprocess(c)`.
  /// Passing a state generated by preprocessing a different collection is
  /// a programming error. You can call `search(_:from:_:)` multiple times with 
  /// the same `state`.
  func search(_ c: C, from s: C.Index, _ state: inout State) -> Range<C.Index>?
}
...
extension CollectionSearcher {
  /// Returns the range of the first match within `c`, if one exists.
...
extension BidirectionalCollectionSearcher {
  /// Returns the range of the last match within `c`, if one exists.
